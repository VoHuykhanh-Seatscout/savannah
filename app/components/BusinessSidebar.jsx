  ...
  >
    ...
    <div className="text-left">
          ...
          {menuItems.map(({ id, label, icon, route }) => (
            ...
            >
              <span className="flex items-center space-x-3">
                <span className={`transition-colors ${activeTab === id ? "text-orange-600" : "text-gray-500"}`}>
                  {icon}
                </span>
                <span>{label}</span>
              </span>
              <FaChevronRight className={`w-3 h-3 transition-all ${activeTab === id ? "opacity-80" : "opacity-40"}`} />
            </button>
          ))}
        </nav>
      </div>
      ...
      <div className="mb-6">
        ...
        <nav className="space-y-1">
          {accountItems.map(({ id, label, icon, route }) => (
            <button
              key={id}
              onClick={() => router.push(route)}
              className={`w-full flex items-center justify-between px-3 py-2.5 rounded-lg transition-all duration-200 ${
